...
class WebBase(FWBase):
    ...
    def get_driver(self):
        if self.manager.driver_area.driver is None:
            self.manager.driver_area.start_driver()
        return self.manager.driver_area.driver
    ...
    @allure.step('Send keys')
    def find_element_by_link_text(self, text, wait=30):
        try:
            return WebDriverWait(self.get_driver(), wait).until(EC.presence_of_element_located(text))
        except:
            pass
